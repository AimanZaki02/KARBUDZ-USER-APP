...
class SignUpApiCall {
  Future<ApiCallResponse> call({
    ...
    String? email = '',
    String? password = '',
    String? registrationToken = 'defaultToken123', // Add a default token
    String? deviceId = '',
    String? token = '',
  }) async {
    final baseUrl = CarServiceGroup.getBaseUrl();
    final fullUrl = '$baseUrl/sign_up';

    // Use a fallback value for registrationToken and deviceId
    final effectiveRegistrationToken = registrationToken ?? 'defaultToken123';
    final effectiveDeviceId = deviceId ?? 'defaultDeviceId';
    ...
    final ffApiRequestBody = '''
{
  ...
  "email": "$email",
  "password": "$password",
  "registrationToken": "$effectiveRegistrationToken",
  ...
  int? success(dynamic response) {
    final successValue = getJsonField(response, r'''$.data.success''');
  ...
  String? message(dynamic response) {
    final messageValue = getJsonField(response, r'''$.data.message''');
...
class CurrencyApiCall {
  Future<ApiCallResponse> call({
    ...
    print('Making API call to: $baseUrl/currency');
  ...
  int? success(dynamic response) => castToType<int>(
        getJsonField(response, r'''$.data.success'''),
      );

  /// Extract message for better debugging
  String? message(dynamic response) => castToType<String>(
        getJsonField(response, r'''$.data.message'''),
      );

  /// Extract currency details
  String? currency(dynamic response) {
    final details =
        getJsonField(response, r'''$.data.currencydetails[0].currency''');
